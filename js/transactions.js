...
function renderTransactions(data) {
  ...
  data.forEach(tx => {
    ...
    row.innerHTML = `
      ...
      <td data-label="Amount">₵${Number(tx.amount || 0).toFixed(2)}</td>
      <td data-label="Bal Before">₵${Number(tx.balance_before ?? 0).toFixed(2)}</td>
      <td data-label="Bal After">₵${Number(tx.balance_after ?? 0).toFixed(2)}</td>
...
function updatePaginationControls() {
  ...
  pageInfo.textContent = `Page ${currentPage} of ${totalPages} (${allTransactions.length} total)`;
}
...
function viewTransactionDetails(transactionId) {
  ...
  modalBody.innerHTML = `
    ...
    <div class="detail-row">
      ...
      <span class="detail-value">₵${Number(tx.balance_before ?? 0).toFixed(2)}</span>
    </div>
    <div class="detail-row">
      <span class="detail-label">Balance After</span>
      <span class="detail-value">₵${Number(tx.balance_after ?? 0).toFixed(2)}</span>
    </div>
    <div class="detail-row">
